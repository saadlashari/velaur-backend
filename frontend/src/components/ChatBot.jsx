  ...
  return (
    <div style={{ background: 'linear-gradient(135deg,#FDF0E8,#FAE8D0)', padding: '1rem', marginTop: '0.5rem' }}>
      ...
      <p onClick={onSwitch} style={{ fontFamily: 'Jost', fontSize: '0.72rem', color: '#F2A7A7', cursor: 'pointer', marginTop: '0.5rem', textAlign: 'center' }}>
        {mode === 'login' ? 'New user? Register' : 'Already have account? Login'}
      </p>
    </div>
  );
}
...
export default function ChatBot({ products = [] }) {
  ...
  const [config, setConfig] = useState({ bot_name: 'Vera' });
  ...
  useEffect(() => {
    getConfig()
      .then(res => {
        setConfig(res.data || { bot_name: 'Vera' });
        setMessages([{
          role: 'assistant',
          content: res.data?.welcome_message || '🌹 Hello! I am Vera, your Velaur assistant!',
  ...
  const handleSend = async (text = null) => {
    ...
    if (msgText.toLowerCase().includes('login') && !user) {
      setLoading(false);
      setAuthMode('login');
      addMessage('assistant', '🔐 Please login:', { showAuthForm: true });
      return;
    }
    if (msgText.toLowerCase().includes('register') && !user) {
      setLoading(false);
      setAuthMode('register');
      addMessage('assistant', '🌹 Create account:', { showAuthForm: true, authMode: 'register' });
      return;
    }
    ...
    try {
      ...
      if (action === 'CART_UPDATED') quickReplies = ['show cart', 'checkout'];
      ...
      addMessage('assistant', '❌ Could not connect to server. Please try again.');
    } finally {
      setLoading(false);
    }
  };
    ...
    try {
      ...
      if (res.data.success) {
        ...
        addMessage('assistant', res.data.message || 'Welcome! 🌹', {
          quickReplies: ['Show products', 'View cart']
        });
      ...
      {open && (
        <div style={isMobile ? {
          ...
          <div style={{
            ...
            {cartCount > 0 && (
              <span style={{ background: '#2C1810', color: '#FAE8D0', padding: '0.2rem 0.6rem', fontFamily: 'Jost', fontSize: '0.7rem' }}>
                🛒 {cartCount}
              </span>
            )}
            <button onClick={() => setOpen(false)} style={{
              background: 'none', border: 'none', fontSize: '1.1rem',
              cursor: 'pointer', color: '#2C1810', padding: '0.2rem 0.4rem',
            }}>✕</button>
          </div>

          {checkoutStep !== 'idle' && (
            <div style={{
              background: '#FDF0E8', padding: '0.35rem 1rem',
              fontFamily: 'Jost', fontSize: '0.68rem', color: '#8B6F5E',
              letterSpacing: '0.1em', textTransform: 'uppercase',
              borderBottom: '1px solid #FAE8D0', flexShrink: 0,
            }}>🔄 Checkout in progress...</div>
          )}
              ...
              <div key={i}>
                <div style={{ display: 'flex', justifyContent: msg.role === 'user' ? 'flex-end' : 'flex-start' }}>
                  <div style={{
                    ...
                    background: msg.role === 'user' ? '#2C1810'
                      : msg.isSuccess ? 'linear-gradient(135deg,#e8f5e9,#f1f8e9)'
                      : 'linear-gradient(135deg,#FDF0E8,#FAE8D0)',
                    color: msg.role === 'user' ? '#FAE8D0' : '#2C1810',
                    fontFamily: 'Jost,sans-serif', fontSize: '0.82rem', lineHeight: 1.65,
                ...
                {msg.showAuthForm && (
                  <AuthForm mode={msg.authMode || authMode} onSubmit={handleAuth}
          ...
          {/* Input */}
          <div style={{
            padding: '0.75rem', borderTop: '1px solid #FAE8D0',
            ...
            <input value={input} onChange={e => setInput(e.target.value)} onKeyDown={handleKey}
              ...
              style={{
                flex: 1, border: '1px solid #FAE8D0', padding: '0.65rem 0.9rem',
                fontFamily: 'Jost', fontSize: '0.85rem',
            ...
            <button onClick={() => handleSend()} disabled={loading || !input.trim()}
              style={{
                ...
                cursor: loading ? 'not-allowed' : 'pointer', minWidth: 44,
              }}>→</button>
          </div>
